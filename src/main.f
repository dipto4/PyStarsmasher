...
*****************************************************************
      ...
      if(nprocs.gt.nprocsmax)then
         write(6,*)'increase nprocsmax in starsmasher.h'
         stop
      endif
      ...
      if(ngravprocs.gt.nprocs)then
         if(myrank.eq.0)then
            write(6,*)'need to have ngravprocs <= nprocs'
            write(6,*)'setting ngravprocs = nprocs = ',nprocs
         endif
         ngravprocs=nprocs
      endif
...
!     the following line assumes ppn cpu threads and abs(ngravprocs) gpu threads per node
         ...
         if(myrank.eq.0) then
            write(6,*)'because ngravprocs<0 in sph.input, we reset it:'
            write(6,*)'ngravprocs=',ngravprocs
         endif
      endif
      
      if(ngravprocs.gt.ngravprocsmax)then
         write(6,*)'increase ngravprocsmax in starsmasher.h'
         stop
      endif
        print *, "do i get to line 212 in main.f?" 
      if(.true.)then
c      if(4*ngravprocs.ge.nprocs)then
c     let myrank<ngravprocs do both gravity and hydro:
         print *, "do i get to line 216 in main.f"
         stride=(n-1)/nprocs+1
         print *, "stride = ", stride
         ...
         do irank=0,nprocs-1
            ...
            if(myrank.eq.0) write(6,*)'myrank,displ,recvcount=',
     ...
     $              irank,gravdispls(irank+1),gravrecvcounts(irank+1),totnuminteractions
         ...
         if(myrank.lt.ngravprocs .and. nusegpus.eq.1) then
            if(qthreads .lt. 0)then
               if(myrank.eq.0) write(6,*)'timing to find best q...'
